...
class DapricotDev(tfds.core.GeneratorBasedBuilder):
    ...
    def _info(self) -> tfds.core.DatasetInfo:
        ...
        # # features["objects"] = features["objects"][0]
            ...
            for image_camera_1, image_camera_2, image_camera_3 in zip(
                ...
                example = {
                    ...
                    "objects": [
                        ...
                        {
                            "id": annos[0]["id"],
                            "image_id": annos[0]["image_id"],
                            "area": annos[0]["area"],
                            "boxes": build_bbox(*annos[0]["bbox"]),
                            "labels": annos[0]["category_id"],
                            "is_crowd": bool(annos[0]["iscrowd"]),
                        }
                        for annos in [
...
class DapricotAnnotation(object):
    ...
    def categories(self):
        ...
        # # this won't work because the key can't be arbitrary...
        # return self._data["info"]["description"], self._data["categories"]
